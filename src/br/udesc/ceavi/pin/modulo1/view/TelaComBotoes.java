...
public class TelaComBotoes extends ViewJanelaSistema implements ObservadorDateNetwork {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -457854104447122854L;
...
new ListenersTelaComBotoes(this, listBtn);
	}
	...
	private void initMyComponents() {
		...
		cons.gridx = 1;
		cons.gridy = 0;
		cons.insets = new Insets(-25, 5, 5, 5);
	...
	public void notifyAlteracao() {
		...
		try {
			...
			dateNetwork.getAllEgde().forEach(egde -> {
				// Render Egde
				areaDesenho.addSpriteDateNetwork("EgdeView", new float[] { egde.x1(), egde.y1(), egde.x2(), egde.y2() },
						Color.GRAY);

				// Render Node De
				areaDesenho.addSpriteDateNetwork("NodeView", new float[] { egde.x1(), egde.y1() }, Color.RED);

				// Render Node Para
				areaDesenho.addSpriteDateNetwork("NodeView", new float[] { egde.x2(), egde.y2() }, Color.RED);
			});
		} catch (Exception e) {
		}
	}
